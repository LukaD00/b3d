...
def b3d(model, c):
	...
	def loss(x, m, p, c, sep=False): 
		...
		if sep:
			...
			#return 0, lambd * torch.linalg.norm(torch.flatten(m),ord=1)
		else:
			return F.cross_entropy(predicted, target) + lambd*torch.linalg.norm(torch.flatten(m),ord=1)
	...
	#theta_m = torch.zeros(3,16,16).to(device)
	#theta_p = torch.zeros(3,16,16).to(device)
	theta_m = torch.full(size=(3,16,16),fill_value=-2.0).to("cuda")
	theta_p = torch.full(size=(3,16,16),fill_value=-2.0).to("cuda")
	#optimizer = torch.optim.Adam((theta_m, theta_p), lr=0.1)		
	optimizer = torch.optim.Adam(({'params':theta_m, 'lr': 0.05}, {'params':theta_p}), lr=0.05)
	...
	with torch.no_grad():
		for _ in range(epochs):
			for inputs, _ in dataloader:
				...
				optimizer.zero_grad()
				theta_m.grad = torch.zeros((3,16,16)).to(theta_m.device)
				theta_p.grad = torch.zeros((3,16,16)).to(theta_p.device)
				...
				for _ in range(k):
					eps = torch.normal(mean=0, std=1, size=(3,16,16)).to(device)
...
if __name__=="__main__":
	...
	model.load_state_dict(torch.load("weights/resnet_cifar_poisoned_scaled.pt"))
	model.eval()
	backdoors = b3d_complete(model, "weights/resnet_cifar_poisoned_scaled-TRIGGERS.pt")
	torch.save(backdoors, "weights/resnet_cifar_poisoned_scaled-BACKDOORS.pt")